...
export type SaleEffectType = Extract<YocoV2EffectType, 'SALE_REPORTING' | 'SALE_STOCK'>;
...
export interface SaleEffectRuntime {
  effectType: SaleEffectType;
  workspaceId: string;
...
export async function getSaleEffectRuntime(
  env: Env,
  workspaceId: string,
  integrationId: string,
  effectType: SaleEffectType,
): Promise<SaleEffectRuntime> {
  ...
  const environmentEnabled = effectType === 'SALE_REPORTING'
    ? flags.yoco_v2_live_sale_reporting
    : flags.yoco_v2_live_sale_stock;
  ...
  const reason = !environmentEnabled ? 'ENVIRONMENT_FEATURE_FLAG_DISABLED'
    : !ownerIsV2 ? 'EFFECT_OWNERSHIP_NOT_V2'
      : !featureEnabled ? 'WORKSPACE_EFFECT_DISABLED'
        : paused ? 'V2_CONSUMPTION_PAUSED'
          : 'ACTIVE';
  return {
...
export async function pauseSaleEffect(env: Env, input: {
  workspaceId: string; integrationId: string; effectType: SaleEffectType; actorId: string; reason?: string;
}): Promise<Row> {
  const now = nowIso();
  ...
  return (await env.DB.prepare(
    `SELECT * FROM yoco_v2_effect_controls WHERE workspace_id=?1 AND integration_id=?2 AND effect_type=?3`,
  ).bind(input.workspaceId, input.integrationId, input.effectType).first<Row>()) || {};
}

export async function resumeSaleEffect(env: Env, input: {
  workspaceId: string; integrationId: string; effectType: SaleEffectType; actorId: string;
}): Promise<Row> {
  const now = nowIso();
  await env.DB.prepare(
    `INSERT INTO yoco_v2_effect_controls
