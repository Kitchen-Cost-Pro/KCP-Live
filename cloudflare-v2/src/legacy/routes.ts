...
import { tenantIdChunks } from '../sql-limits';
...
async function assertRecipeIngredientIdsAllowed(env: Env, workspaceId: string, recipeLines: Array<{ stockItemId: string }>) {
  ...
  const rows = await env.DB.prepare(
    `SELECT id, name, item_type, active, raw_json
       FROM stock_items
      WHERE workspace_id = ?1
        AND id IN (${placeholders})`
  ).bind(workspaceId, ...ids).all<Record<string, unknown>>();
  const foundIds = new Set((rows.results || []).map((row) => text(row.id)).filter(Boolean));
  const missingId = ids.find((stockItemId) => !foundIds.has(stockItemId));
  if (missingId) {
    throw new Error(`Recipe ingredient ${missingId} could not be found in this workspace.`);
  }
  const inactive = (rows.results || []).find((row) => Number(row.active ?? 1) === 0);
  if (inactive) {
    throw new Error(`${text(inactive.name || inactive.id)} is inactive and cannot be assigned as a recipe ingredient.`);
  }
...
async function saveProductRecipe(
  ...
  productId: string,
  recipe: unknown[],
  options: { recipeSourceStockItemId?: string } = {}
) {
  ...
  const product = await env.DB.prepare(
    `SELECT id, recipe_source_stock_item_id, raw_json
       FROM products
      WHERE workspace_id = ?1
        AND id = ?2
        AND active = 1
      LIMIT 1`
  ).bind(workspaceId, productId).first<{
    id: string;
    recipe_source_stock_item_id?: string | null;
    raw_json?: string | null;
  }>();
  if (!product?.id) throw new Error('Menu item could not be found.');

  const shouldUpdateRecipeSource = Object.prototype.hasOwnProperty.call(options, 'recipeSourceStockItemId');
  const recipeSourceStockItemId = shouldUpdateRecipeSource
    ? text(options.recipeSourceStockItemId)
    : text(product.recipe_source_stock_item_id);
  if (recipeSourceStockItemId) {
    const sourceItem = await env.DB.prepare(
      `SELECT id
         FROM stock_items
        WHERE workspace_id = ?1
          AND id = ?2
          AND active = 1
        LIMIT 1`
    ).bind(workspaceId, recipeSourceStockItemId).first<{ id: string }>();
    if (!sourceItem?.id) throw new Error('The linked recipe source stock item could not be found.');
  }
  ...
  const linkedRecipeExists = !recipeLines.length && recipeSourceStockItemId
    ? await recipeHasLines(env, workspaceId, 'stock_item', recipeSourceStockItemId)
    : false;
  const missingRecipe = recipeLines.length || linkedRecipeExists ? 0 : 1;
  const persistedRecipeShape = recipeLines.map((line) => ({
  ...
  const nextRawJson = JSON.stringify({
    ...objectValue(jsonParse(product.raw_json)),
    recipe: persistedRecipeShape,
    recipeLines: persistedRecipeShape,
    recipeSourceStockItemId,
    recipe_source_stock_item_id: recipeSourceStockItemId,
    missingRecipe: missingRecipe === 1,
    updatedAt: now
  });

  if (shouldUpdateRecipeSource) {
    statements.push(env.DB.prepare(
      `UPDATE products
          SET recipe_source_stock_item_id = ?3,
              missing_recipe = ?4,
              raw_json = ?5,
              updated_at = ?6
        WHERE workspace_id = ?1
          AND id = ?2`
    ).bind(workspaceId, productId, recipeSourceStockItemId || null, missingRecipe, nextRawJson, now));
  } else {
    statements.push(env.DB.prepare(
      `UPDATE products
        ...
        WHERE workspace_id = ?1
          AND id = ?2`
    ).bind(workspaceId, productId, missingRecipe, nextRawJson, now));
  }

  await env.DB.batch(statements);

  const [persistedRows, persistedProduct] = await Promise.all([
    env.DB.prepare(
      `SELECT stock_item_id, quantity, unit, sort_order
         FROM recipe_lines
        WHERE workspace_id = ?1
          AND recipe_id = ?2
        ORDER BY sort_order ASC`
    ).bind(workspaceId, recipeId).all<Record<string, unknown>>(),
    env.DB.prepare(
      `SELECT recipe_source_stock_item_id, missing_recipe, raw_json, updated_at
         FROM products
        WHERE workspace_id = ?1
          AND id = ?2
        LIMIT 1`
    ).bind(workspaceId, productId).first<Record<string, unknown>>()
  ]);
  const persistedRecipe = arrayValue(persistedRows.results).map((entry) => {
    const row = objectValue(entry);
    return {
      ingId: text(row.stock_item_id),
      stockItemId: text(row.stock_item_id),
      qty: numberValue(row.quantity, 0),
      quantity: numberValue(row.quantity, 0),
      unit: text(row.unit, 'ea') || 'ea'
    };
  });
  const persistedRaw = objectValue(jsonParse(persistedProduct?.raw_json));
  const persistedRawRecipe = normalizeProductRecipeLines(arrayValue(persistedRaw.recipe || persistedRaw.recipeLines));
  const relationalMatches = recipeLinesEqual(recipeLines, persistedRecipe);
  const rawJsonMatches = recipeLinesEqual(recipeLines, persistedRawRecipe);
  const sourceMatches = text(persistedProduct?.recipe_source_stock_item_id) === recipeSourceStockItemId;
  if (!persistedProduct || !relationalMatches || !rawJsonMatches || !sourceMatches) {
    throw new Error('The recipe write did not match the database read-back. No success was reported.');
  }
  ...
  return {
    ...
    recipeSourceStockItemId,
    recipeStatus: missingRecipe ? 'MISSING_RECIPE' : linkedRecipeExists ? 'COMPLETE_VIA_LINKED_STOCK_ITEM' : 'COMPLETE',
    persisted: true,
    persistedAt: text(persistedProduct.updated_at || now),
    storageVerified: {
      recipeLines: true,
      productJson: true
    }
...
function recipeLinesEqual(
  expected: Array<{ stockItemId: string; quantity: number; unit: string }>,
  actual: unknown[]
) {
  const normalizedActual = normalizeProductRecipeLines(actual);
  if (expected.length !== normalizedActual.length) return false;
  return expected.every((line, index) => {
    const other = normalizedActual[index];
...
async function readProductRecipe(env: Env, workspaceId: string, productId: string) {
  const product = await env.DB.prepare(
    `SELECT id, recipe_source_stock_item_id, missing_recipe, raw_json, updated_at
       FROM products
      WHERE workspace_id = ?1
        AND id = ?2
        AND active = 1
  ...
  const recipeLines = arrayValue(rows.results).map((entry) => {
    const row = objectValue(entry);
    return {
      ingId: text(row.stock_item_id),
      stockItemId: text(row.stock_item_id),
  ...
  const raw = objectValue(jsonParse(product.raw_json));
  const rawRecipe = normalizeProductRecipeLines(arrayValue(raw.recipe || raw.recipeLines));
  const storageConsistent = recipeLinesEqual(
    normalizeProductRecipeLines(recipeLines),
    rawRecipe
  );
  ...
  return {
    ...
    recipeSourceStockItemId,
    recipeStatus,
    persistedAt: text(product.updated_at),
    persisted: true,
    storageVerified: {
      recipeLines: true,
      productJson: storageConsistent
    }
  };
}
...
async function saveYocoModifierRecipe(env: Env, workspaceId: string, ownerId: string, recipe: unknown[], linkedProductId?: string) {
  ...
  await env.DB.batch(statements);
  const [persistedRecipe, persistedLines] = await Promise.all([
    env.DB.prepare(
      `SELECT linked_product_id
        ...
        WHERE workspace_id = ?1
          ...
          AND active = 1
        LIMIT 1`
    ).bind(workspaceId, recipeId, ownerId).first<{ linked_product_id?: string | null }>(),
    env.DB.prepare(
  ...
  const persistedRecipeLines = arrayValue(persistedLines.results).map((entry) => {
    const row = objectValue(entry);
    return {
      ingId: text(row.stock_item_id),
      stockItemId: text(row.stock_item_id),
  ...
  if (!persistedRecipe || persistedRecipeLines.length !== recipeLines.length) {
    throw new Error('The modifier recipe save could not be verified.');
  }
  return {
    recipeId,
    recipe: persistedRecipeLines,
    linkedProductId: text(persistedRecipe.linked_product_id),
    persisted: true
  };
}
...
async function saveStockItemRecipe(env: Env, workspaceId: string, stockItemId: string, raw: Record<string, unknown>, item: ReturnType<typeof normalizeStockItemPayload>) {
  ...
  if (arrayValue(persisted.results).length !== recipeLines.length) {
    throw new Error('The stock item recipe save completed, but the saved ingredient count could not be verified.');
  }
}
...
export async function getStockItems(request: Request, env: Env, auth: AuthContext, workspaceId: string) {
  ...
  if (stockItemIds.length) {
    ...
    for (const stockItemIdChunk of tenantIdChunks(stockItemIds)) {
      const idPlaceholders = stockItemIdChunk.map((_, index) => `?${index + 2}`).join(', ');
      ...
      ).bind(workspaceId, ...stockItemIdChunk).all<Record<string, unknown>>();
      recipes.push(...((recipeRows.results || []) as Record<string, unknown>[]));
    }
    const recipeIds = recipes.map((recipe) => text(recipe.id)).filter(Boolean);
    const linesByRecipe = new Map<string, Record<string, unknown>[]>();
    if (recipeIds.length) {
      for (const recipeIdChunk of tenantIdChunks(recipeIds)) {
        const recipePlaceholders = recipeIdChunk.map((_, index) => `?${index + 2}`).join(', ');
        const lineRows = await env.DB.prepare(
          `SELECT recipe_id, stock_item_id, quantity, unit, sort_order
            ...
            ORDER BY sort_order ASC`
        ).bind(workspaceId, ...recipeIdChunk).all<Record<string, unknown>>();
...
export async function patchProduct(request: Request, env: Env, auth: AuthContext, workspaceId: string, productId: string) {
  await scoped(request, env, auth, workspaceId);
  const payload = await readJson<Record<string, unknown>>(request);
  const productPatch = objectValue(payload.product || payload.item || payload);
  const recipeWasSupplied = Object.prototype.hasOwnProperty.call(productPatch, 'recipe');
  const recipeSourceWasSupplied = Object.prototype.hasOwnProperty.call(productPatch, 'recipeSourceStockItemId') ||
    Object.prototype.hasOwnProperty.call(productPatch, 'recipe_source_stock_item_id');

  // Recipe editor writes intentionally use the established product PATCH URL so an updated Pages
  // build remains compatible with an older Worker during deployment. Keep the write recipe-only:
  // do not rewrite product name, pricing or raw catalogue metadata for this request.
  const recipeOnlyFields = new Set(['recipe', 'recipeSourceStockItemId', 'recipe_source_stock_item_id']);
  const isRecipeOnlyPatch = recipeWasSupplied &&
    Object.keys(productPatch).every((key) => recipeOnlyFields.has(key));
  if (isRecipeOnlyPatch) {
    const cleanProductId = routeText(productId);
    if (!cleanProductId) return error(request, env, 400, 'Menu item id is required.');
    const result = await saveProductRecipe(
      env,
      workspaceId,
      cleanProductId,
      arrayValue(productPatch.recipe),
      recipeSourceWasSupplied
        ? { recipeSourceStockItemId: text(productPatch.recipeSourceStockItemId || productPatch.recipe_source_stock_item_id) }
        : {}
    );
    try {
      await env.DB.prepare(
        `INSERT INTO audit_events
          (id, workspace_id, actor_uid, event_type, entity_type, entity_id, after_json, created_at)
         VALUES (?1, ?2, ?3, 'recipe_saved', 'product_recipe', ?4, ?5, ?6)`
      ).bind(id('audit'), workspaceId, auth.uid, cleanProductId, JSON.stringify({
        recipe: result.recipe,
        recipeSourceStockItemId: result.recipeSourceStockItemId,
        recipeStatus: result.recipeStatus
      }), nowIso()).run();
    } catch (auditError) {
      console.warn('Recipe saved but its audit event could not be recorded.', auditError);
    }
    return json(request, env, {
      ok: true,
      id: cleanProductId,
...
export async function putProductRecipe(request: Request, env: Env, auth: AuthContext, workspaceId: string, productId: string) {
  await scoped(request, env, auth, workspaceId);
  const cleanProductId = routeText(productId);
  if (!cleanProductId) return error(request, env, 400, 'Menu item id is required.');
  const payload = objectValue(await readJson<Record<string, unknown>>(request));
  const recipeSourceWasSupplied = Object.prototype.hasOwnProperty.call(payload, 'recipeSourceStockItemId') ||
    Object.prototype.hasOwnProperty.call(payload, 'recipe_source_stock_item_id');
  const result = await saveProductRecipe(
    env,
    workspaceId,
    cleanProductId,
    ...
    recipeSourceWasSupplied
      ? { recipeSourceStockItemId: text(payload.recipeSourceStockItemId || payload.recipe_source_stock_item_id) }
      : {}
  );
  const now = nowIso();
  try {
...
export async function getProductRecipe(request: Request, env: Env, auth: AuthContext, workspaceId: string, productId: string) {
  await scoped(request, env, auth, workspaceId);
  const cleanProductId = routeText(productId);
  if (!cleanProductId) return error(request, env, 400, 'Menu item id is required.');
  const result = await readProductRecipe(env, workspaceId, cleanProductId);
  return json(request, env, {
    ok: true,
    id: cleanProductId,
    ...result
...
export async function patchYocoModifierRecipe(request: Request, env: Env, auth: AuthContext, workspaceId: string, ownerId: string) {
  ...
  const payload = objectValue(await readJson<Record<string, unknown>>(request));
  const result = await saveYocoModifierRecipe(
    env,
    workspaceId,
    cleanOwnerId,
    arrayValue(payload.recipe),
