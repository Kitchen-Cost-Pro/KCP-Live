...
export function normalizeApiSalesFinancialRow(row = {}, index = 0, meta = {}) {
  ...
  return {
    ...
    receiptNumber: text(row.receiptNumber || row.receipt_number || row.parentYocoOrderId || row.parent_yoco_order_id || row.yocoOrderId || row.yoco_order_id || row.orderNumber),
    ...
    refundGrossAmount: safeNumber(row.refundGrossAmount ?? row.refund_gross_amount ?? row.refundAmount ?? row.refund_amount),
    refundVatAmount: safeNumber(row.refundVatAmount ?? row.refund_vat_amount),
    refundNetAmount: safeNumber(row.refundNetAmount ?? row.refund_net_amount),
...
export function normalizeApiModifierSalesRow(row = {}, index = 0, meta = {}) {
  ...
  return {
    ...
    modifierType: text(row.modifierType || row.modifier_type) || 'Option',
    ...
    stockActionType: text(row.stockActionType || row.stock_action_type || row.modifierActionType || row.modifier_action_type || row.noteActionType || row.note_action_type).toUpperCase(),
    stockAction: text(row.stockAction || row.stock_action),
    stockQtyDeducted,
