...
function normalizeModifierRow(row = {}) {
  ...
  const explicitVatUsable = hasModifierValue(explicitVat) && (explicitVatAmount > 0 || !grossAmount || isVatExempt);
  ...
  return {
    ...
    qty: safeNumber(row.qty ?? row.timesSelected, 1),
    timesSelected: safeNumber(row.timesSelected ?? row.qty, 1),
    grossAmount,
    vatAmount,
    netAmount,
    vatRate,
...
function validateModifierRows(rows = []) {
  ...
  return [
    ...
    warning(modifierRows, (row) => row.hasModifierUsage === true && safeNumber(row.stockQtyDeducted ?? row.stockDeducted) === 0, 'modifier-usage-qty-missing', 'critical', 'Modifier Usage row has no qty.'),
