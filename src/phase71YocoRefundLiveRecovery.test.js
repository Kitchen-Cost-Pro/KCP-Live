...
test('Phase 71 exposes the deployed Worker release so Pages and API drift is visible', () => {
  ...
  assert.match(release, /phase7[123]-yoco-(?:refund-live-recovery|webhook-rate-limit-safe|refund-original-order-resolution)/);
  assert.match(worker, /\/api\/runtime-version/);
  assert.match(worker, /workerRelease: KCP_WORKER_RELEASE/);
  assert.match(admin, /KCP_EXPECTED_WORKER_RELEASE = 'phase7[123]-yoco-(?:refund-live-recovery|webhook-rate-limit-safe|refund-original-order-resolution)'/);
