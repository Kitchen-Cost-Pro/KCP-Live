...
## Current Yoco V2 behavior

- The external Yoco webhook has one V2 ingress.
- Raw events are persisted before queue processing.
- Queue processing resolves canonical sale and refund events.
- Sale and refund reporting effects write canonical Yoco reporting records.
- Inventory effects write deterministic rows to `stock_movements`.
- Financial refund completion is independent from inventory allocation. Unresolved amount-only refunds remain visible and require manual stock allocation.
- All remote Yoco requests use the V2 rate-gated client and honour `Retry-After`.
- Reconciliation uses the same canonical processors and idempotent effect writers as webhook processing.
- Every active workspace must have enabled V2 ownership for `SALE_REPORTING`, `SALE_STOCK`, `REFUND_REPORTING`, and `REFUND_STOCK`.
- The runtime has no legacy sale/refund processor or rollback-to-LEGACY switch.

See `docs/yoco-v2/README.md` for the current architecture and production-readiness gates.
